# DiLoCo Distributed Low-Communication Training

A deterministic NumPy reference for local inner optimization, pseudo-gradient aggregation, and momentum outer updates.

## Layout

- `src/diloco/core.py`: deterministic, modular implementation.
- `src/diloco/cli.py`: configuration-driven command line entry point.
- `tutorial.ipynb`: executable first-principles walkthrough.
- `tests/test_core.py`: behavioral and numerical checks.
- `config.json`: small reproducible default run.

## Run

```bash
python -m pip install -e '.[test]'
diloco --config config.json
pytest
```

The default configuration is deliberately small enough for CPU execution. Scale dimensions and steps only after the reference assertions pass.
